...
public class road_ab_with_dbg implements Runnable {
	...
	private void solve() throws IOException {
			...
			for (int j = i + 1; j < 4; ++j) {
				...
				if (pc[0] != 0) {
					...
					if (td1 == td2) {
						...
						double tx = pc[1] / 2.0 / pc[0];
						double ty = pc[2] / 2.0 / pc[0];
						double tr = (Math.hypot(x[i] / 2.0 - tx, y[i] / 2.0 - ty) + Math.hypot(x[k] / 2.0 - tx, y[k] / 2.0 - ty)) / 2.0;
						for (int z = 0; z < 4; ++z) {
							System.out.print((Math.hypot(x[z] / 2.0 - tx, y[z] / 2.0 - ty) - tr) + " ");
						}
						System.out.println();
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (td1 == td2) {
				...
				double tx = pc[1] / 2.0 / pc[0];
				double ty = pc[2] / 2.0 / pc[0];
				double tr = (Math.hypot(x[i] / 2.0 - tx, y[i] / 2.0 - ty) + Math.hypot(x[a] / 2.0 - tx, y[a] / 2.0 - ty)) / 2.0;
	...
	public static void main(String[] args) {
		new Thread(new road_ab_with_dbg()).start();
	}
