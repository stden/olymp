...
public class preference_pm_slow implements Runnable {
    ...
    public static void main(String[] args) {
        new Thread(new preference_pm_slow()).start();
    }
    ...
    static class Position {
        ...
        public boolean equals(Object o) {
            ...
            if (alpha != position.alpha) return false;
            if (beta != position.beta) return false;
    ...
    private void solve() {
        ...
        cache = new HashMap<Position, Integer>();
                ...
                if (start.getOwner(d1) == 0)
                    for (int d2 : dismissTryOrder)
                        if (d1 < d2 && start.getOwner(d2) == 0) {
                            ...
                            first.trump = trump;
                            first.alpha = (byte) Math.max(res - 1, 0);

                            int value = evaluate(first);
                            if (value > res) {
                                res = value;
                                winningGames.clear();
                            }
                                ...
                                for (int c3 : moveTryOrder)
                                    if (p.getOwner(c3) == p3)
                                        if (validTurn(c1, p.trump, c3, can3trump, can3offsuit)) {
                                            ...
                                            adjustAlphasBetas(next, b3, p3);
                                            adjustAlphasBetas(next, b2, p2);
                                            adjustAlphasBetas(next, b1, p1);
                                            next.alpha -= added;
                                            next.beta -= added;
                                            next.alpha = (byte) Math.max(next.alpha, 0);
                                            next.beta = (byte) Math.min(next.beta, countCards(next));
                                            int cur = added + evaluate(next);
                                            if (p3 == 0)
    ...
    private void adjustAlphasBetas(Position p, byte curBest, int who) {
        if (who == 0) {
            p.alpha = (byte) Math.max(p.alpha, curBest);
        } else {
            p.beta = (byte) Math.min(p.beta, curBest);
        }
    }
