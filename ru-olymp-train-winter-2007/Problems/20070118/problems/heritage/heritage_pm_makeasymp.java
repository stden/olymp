...
public class heritage_pm_makeasymp {
    final String TASK_ID = "heritage";
    final String OUT_FILE = TASK_ID + ".asy";
    ...
    static class Enumeration {
        ...
        public boolean equals(Object o) {
            heritage_pm_makeasymp.Enumeration that = (heritage_pm_makeasymp.Enumeration) o;
    ...
    private void solve() {
        ...
        for (int i = 0; i <= 2 * NUM_RECTS; ++i)
            for (int j = 0; j <= 2 * NUM_RECTS; ++j)
                res += resCnt[i][j] * c(1e30, i) * c(1e30, j);
        for (int i = 0; i < 2 * NUM_RECTS; ++i)
             res /= 1e60;
    }

    private double c(double n, int k) {
        double res = 1;
        for (int i = 0; i < k; ++i)
            res = res * (n - i) / (i + 1);
        return res;
    ...
    private void countIt() {
        ...
        heritage_pm_makeasymp.Enumeration[] all = possibleEnumerations.keySet().toArray(new heritage_pm_makeasymp.Enumeration[0]);
        int[] amount = new int[nEnum];
        for (int i = 0; i < nEnum; ++i)
            amount[i] = possibleEnumerations.get(all[i]);
        int[] givesMask = buildMasks(nEnum, all);
        Map<heritage_pm_makeasymp.Enumeration, Integer> subEnums = new HashMap<Enumeration, Integer>();
        for (int i = 0; i < nEnum; ++i) {
            if (i % 10 == 0)
                System.out.println(i);
            heritage_pm_makeasymp.Enumeration xE = all[i];
            subEnums.clear();
            int[] rectPerm = new int[NUM_RECTS];
            boolean[] rectUsed = new boolean[NUM_RECTS];
            rec3(0, rectPerm, rectUsed, xE, subEnums);
            for (heritage_pm_makeasymp.Enumeration xSub : subEnums.keySet()) {
                double xAmt = amount[i] / (subEnums.size());
                int xMask = subEnums.get(xSub);
                for (int j = 0; j < nEnum; ++j) {
                    heritage_pm_makeasymp.Enumeration yE = all[j];
    ...
    private int countEquals(heritage_pm_makeasymp.Enumeration e) {
        int cnt = 0;
        for (int j = 0; j < 2 * NUM_RECTS; ++j)
            if (!e.equal[j])
                ++cnt;
    ...
    private void rec3(int num, int[] rectPerm, boolean[] rectUsed, heritage_pm_makeasymp.Enumeration xE, Map<heritage_pm_makeasymp.Enumeration, Integer> subEnums) {
        if (num >= NUM_RECTS) {
    ...
    private void process3(int[] rectPerm, heritage_pm_makeasymp.Enumeration xE, Map<heritage_pm_makeasymp.Enumeration, Integer> subEnums) {
        int[] np = new int[2 * NUM_RECTS];
        for (int i = 0; i < 2 * NUM_RECTS; ++i) {
            int z = xE.perm[i];
        ...
        for (int i = 1; i < 2 * NUM_RECTS; ++i)
            if (xE.equal[i] && np[i] < np[i - 1])
                return;
        heritage_pm_makeasymp.Enumeration e = new heritage_pm_makeasymp.Enumeration(np, xE.equal);
    ...
    int buildMask(heritage_pm_makeasymp.Enumeration e) {
        int mask = 0;
    ...
    private int[] buildMasks(int nEnum, heritage_pm_makeasymp.Enumeration[] all) {
        int[] mask = new int[nEnum];
        for (int i = 0; i < nEnum; ++i) {
            heritage_pm_makeasymp.Enumeration e = all[i];
            mask[i] = buildMask(e);
        }
        return mask;
    }
    ...
    private void buildPossibleEnumerations() {
        ...
        possibleEnumerations = new HashMap<heritage_pm_makeasymp.Enumeration, Integer>();
        totalCnt = 0;
        rec(0, perm, used);
    ...
    private void process2(int[] perm, boolean[] equal) {
        heritage_pm_makeasymp.Enumeration e = new heritage_pm_makeasymp.Enumeration(perm, equal);
        ++totalCnt;
        if (totalCnt % 1000 == 0)
