...
Begin
  ...
  repeat
  	outer1 := TTree.CreateRandom(25);
  	outer2 := outer1.duplicate();
  	first := TTree.CreateSubstitution(TTree.CreateSubstitution(TTree.CreateRandom(10), TTree.CreateStar(4)), outer1);
  	second := TTree.CreateSubstitution(TTree.CreateSubstitution(TTree.CreateRandom(15), TTree.CreateStar(4)), outer2);
  until (first.n <= 500) and (second.n <= 500)and (first.n >= 300) and (second.n >= 400);
  first.writeout;
  second.writeout;
End.