...
Begin
  ...
  repeat
  	outer1 := TTree.CreateRandom(random(100) + 1);
  	outer2 := outer1.duplicate();
  	first := TTree.CreateSubstitution(TTree.CreateFullSubstitution(TTree.CreateRandom(random(10) + 1), TTree.CreateStar(random(8) + 2)), outer1).shuffle;
  	second := TTree.CreateSubstitution(TTree.CreateFullSubstitution(TTree.CreateRandom(random(20) + 1), TTree.CreateStar(random(10) + 2)), outer2).shuffle;
  until (first.n <= 500) and (second.n <= 500)and (first.n >= 350) and (second.n >= 450);
  first.writeout;
  second.writeout;
End.