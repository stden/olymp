...
var
  i,j,i1,k : longint;
  n,t : int64;
  a,a1,b : array [0..500,0..500] of long;
  ot : long;
  ...
  for i:=1 to n do
    for j:=1 to n do
      for i1:=t to min(2*t,i) do
        if not ((a1[i-i1,j-1][0]=1) and (a1[i-i1,j-1][1]=0)) then
          a1[i,j]:=longsum(a1[i,j],a1[i-i1,j-1]);
  for i:=1 to n do
    b[1,i]:=a[n,i];
  for i:=2 to min(n,12) do
    for j:=1 to n do
      for i1:=1 to n do
        if not ((a1[i1,j][0]=1) and (a1[i1,j][1]=0)) then
          b[i,j]:=longsum(b[i,j],longumn(b[i-1,i1],a1[i1,j])); 
  ot[0]:=1; ot[1]:=0;
  for i:=1 to n do
    ot:=longsum(ot,b[i,1]);
  vyvod(ot);
