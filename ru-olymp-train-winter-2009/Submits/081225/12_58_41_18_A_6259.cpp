	...
	while(true)
	{
		while( !q.empty() )
		{
			...
			if( len1 > len2 )
			{
				...
				{
					v.push_back( 
					make_pair(minP,s.substr(0,v1[i]) + s.substr(v1[i]+len1,s.length()))
					);
				}
			} else
			{
				...
				{
					v.push_back( 
					make_pair(minP,s.substr(0,v2[i]) + s.substr(v2[i]+len2,s.length()))
					);
				}
			}
		}//while
		...
		{
			if( v[i].first == minP ) q.push( v[i].second );
		}	
		v.clear();
	}
