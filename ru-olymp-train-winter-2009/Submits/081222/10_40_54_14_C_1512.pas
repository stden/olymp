...
var
  ...
  i,j:longint;
  res:Tlong;
  help:array[0..300,0..300]of longint;
  sumhelp,ssum:longint;
  L1,L2,L3,L0:Tlong;
...
begin
  ...
  rewrite(output);
  {for i:=1 to 5 do if (i mod 3 = 0 ) then help[i][5]:=0 else help[i][5]:=1;
  sumhelp:=0;
  for i:=1 to 5 do begin
    write(help[i][5],' ');
    sumhelp:=sumhelp+help[i][5];
  end;
  for j:=5 downto 1 do if (help[j][5]=1)
      then dec(sumhelp)
      else break;
  ssum:=sumhelp;
  writeln('@@@ ',sumhelp,' |');
  for i:=4 downto 1 do begin
    ...
    for j:=1 to i do begin
      help[j][i]:=help[j][i+1]+help[j+1][i+1];
      help[j][i]:=help[j][i] mod 2;
      write(help[j][i],' ');
      sumhelp:=sumhelp+help[j][i];
    end;
    for j:=i downto 1 do if (help[j][i]=1)
      then dec(sumhelp)
      else break;
    writeln('@@@ ',sumhelp,' |');
    inc(ssum,sumhelp);
