...
var r1,r2:array[0..1000000]of integer;
    nado:array[0..100001]of integer;
    sum,ans,ans1,a:array[0..100001]of int64;
    bc:array[0..100001,0..mt+1]of byte;
    bc1:array[0..10001,0..mt1+1]of integer;
    i,n,j,mn:longint;
    s:string;
    tek,kl,rt:int64;
...
begin
     ...
     if n>10000 then begin
       ...
       for i:=1 to mt1 do ans[i]:=-1;
       ans[1]:=0;
       for i:=2 to n do begin
           for j:=1 to mt1 do ans1[j]:=-1;
           for j:=1 to mt1 do
             if ans[j]<>-1 then begin
                if (ans[j]+sum[i-1]+a[i]<ans1[1])or(ans1[1]=-1) then begin
                   ans1[1]:=ans[j]+sum[i-1]+a[i];
           ...
           for j:=1 to mt1 do ans[j]:=ans1[j];
       end;
       rt:=1;
       kl:=1;
       mn:=1;
       for i:=2 to mt1 do if (ans[i]<>-1)and(ans[i]<ans[mn]) then mn:=i;
       bct1(mn,n);
       tek:=1;
       for i:=2 to n do begin
         if nado[i]=0 then begin
