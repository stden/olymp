...
//    cout << i1 << " " << j1 << " " << i2 << " " << j2 << " " << i3 << " " << j3 << " " << i4 << " " << j4 << " " << mm << endl;
    if (!s2 && !s4) break;
    if (mm) {
      int ch = check (i3, j3, i2, j2, ta, i1, j1, i4, j4);
//      cout << ch << endl;
      int ok = 0;
      for (int i = 0; i < 8; i++) {
	if (cool (i3 + di[i], j3 + dj[i], i1, j1, s2 * i2 + (1 - s2) * 100, s2 * j2 + (1 - s2) * 8, s4 * i4 + (1 - s4) * 100, s4 * j4 + (1 - s4) * 8)) {
	  if (!check (i3 + di[i], j3 + dj[i], s2 * i2 + (1 - s2) * 100, ta, s2 * j2 + (1 - s2) * 8, i1, j1, s4 * i4 + (1 - s4) * 100, s4 * j4 + (1 - s4) * 8)) {
	    ok = 1;
	...
	if (cool (i3 + di[i], j3 + dj[i], i1, j1, -1, -1, i4, j4)) {
//	  cout << i3 + di[i] << " " << j3 + dj[i] << " " << i1 << " " << j1 << endl;
	...
	while (1) {
      ...
      int ch = check (i1, j1, i4, j4, tb, i3, j3, i2, j2);
      int ok = 0;
      for (int i = 0; i < 8; i++) {
	if (cool (i1 + di[i], j1 + dj[i], i3, j3, s2 * i2 + (1 - s2) * 100, s2 * j2 + (1 - s2) * 8, s4 * i4 + (1 - s4) * 100, s4 * j4 + (1 - s4) * 8)) {
	  if (!check (i1 + di[i], j1 + dj[i], i4, j4, tb, i3, j3, s2 * i2 + (1 - s2) * 100, s2 * j2 + (1 - s2) * 8)) {
	    ok = 1;
	    if (!was (i1, j1, i2, j2, s2, i3 + di[i], j3 + dj[i], i4, j4, s4, mm)) {
	      qi1[r] = i1 + di[i];
	      qj1[r] = j1 + dj[i];
	...
	if (cool (i1 + di[i], j1 + dj[i], i3, j3, -1, -1, i2, j2)) {
