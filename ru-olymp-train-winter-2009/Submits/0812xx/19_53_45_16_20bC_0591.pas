...
var N,i,j,l,qS,qF: longint;
    ...
    A,S,St,So,C,Cc,B,Y1,Y2,Y3,Ao,Bo: string;
    NZ: boolean;
    R: longint;
    Bm: array [0..9,-1..255] of longint;
...
function NShorten(P: string): string;
var R: string;
    e,eS,eF: longint;
begin
    R:='';
...
begin
   ...
   A:=chr(random(8)+ord('0')+1);
   B:=chr(random(8)+ord('0')+1);
   for i:=1 to random(20)do
     A:=A+chr(random(9)+ord('0'));
   for i:=1 to random(random(20))do
     B:=B+chr(random(9)+ord('0'));
   ...
   while i+length(Bo)<=length(Ao) {less(B,S)} do
   begin
   ...
   begin
     C:='';
     for j:=1 to length(cut(Y[i+1])) do
       if (Y[i+1][j]=' ')and(Y[i][j]=' ')
       then
         C:=C+' '
       else
       begin
     ...
     for j:=length(cut(Y[i+1]))+1 to length(Y[i+1]) do
       C:=C+' ';
   ...
   begin
     ...
     for j:=1 to length(Y[i-1]) do
       if ((Y[i-1][j]<>' ')or(Y[i+1][j]<>' ')or(Y[i-2][j]<>' '))
       then
       begin
         qS:=j;
         break;
