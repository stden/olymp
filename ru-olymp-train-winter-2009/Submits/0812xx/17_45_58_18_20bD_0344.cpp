	...
	while( nextStr() != "---" )
	{
		...
		if( nextStr() == "F" )
		{
			...
			if( nextStr() == "I" )
			{
				...
				{
					string y = id_first.substr( id_first.find("-")+1, id_first.length() );
					string x = id_first.substr( 0, id_first.find("-") );
					
					if( x.length() == y.length() && isInt(x) && isInt(y) )
					{
						id_first = x;
						id_last = y;
					}
