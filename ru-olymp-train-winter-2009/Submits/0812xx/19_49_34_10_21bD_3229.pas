...
string s;
long long sec[4],speed,download,upload,total[2];
...
int main(){
	...
	while(getline(cin,s)){
		...
		if(type==established+1 || ( type == 3 && established == 3 ) || ( type == 4 && established == 2)){
			...
			if(type==1){//established
				...
				printf("   %lld %lld " ,((sec[2]-sec[1]+24*60*60)%(24*60*60)),speed);
	...
	printf("Total seconds to pay = %lld, total seconds = %lld; total bytes %lld/%lld",sec[3],sec[0],total[0],total[1]);
	return 0;
}
