...
begin
    ...
    if mo=1 then mos:='Jan';
    if mo=2 then mos:='Feb';
    if mo=3 then mos:='Mar';
    if mo=4 then mos:='Apr';
    if mo=5 then mos:='May';
    if mo=6 then mos:='Jun';
    if mo=7 then mos:='Jul';
    if mo=8 then mos:='Aug';
    if mo=9 then mos:='Sep';
    if mo=10 then mos:='Oct';
    if mo=11 then mos:='Nov';
    if mo=12 then mos:='Dec';
    result:=copy(result,1,3)+mos+copy(result,7,length(s)-6);
end;
begin
