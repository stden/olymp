...
vertex a[500000];
int left[500000],right[500000],st,n;
...
int _get(int l,int r,int i){
	...
	a[i].plus=0;
	if(i*2<st+n)if(i*2<st+n)uptodate(i*2);
	if(i*2+1<st+n)if(i*2+1<st+n)uptodate(i*2+1);
...
void check_max(int k,int i){
	...
	if(i*2<st+n)check_max(k,i*2);
	if(i*2+1<st+n)check_max(k,i*2+1);
	uptodate(i);
}
