uses Math;
const osn = 10000;
type digit = array [0..40] Of Longint;
tuk = ^digit;
var
  uka: array [0..500, 0..500] of digit;
  a: array [0..500, 0..500] of tuk;

  ukb, ukb1, ukc, ukc1, ukc2: array [1..500] of digit;
  b, b1, c, c1, c2: array [1..500] of tuk;
  ...
  For i := 1 To n Do
    For j := 1 To m Do
      Begin
        ...
        c^[i+j] := c^[i+j] + (c^[i+j-1] Div osn);
        c^[i+j-1] := c^[i+j-1] mod osn;
      End;
  c^[0] := n+m;
  while (c^[0] > 1) and (c^[c^[0]] = 0) do dec(c^[0]);
End;
    ...
    For i := 1 To n Do
      Begin
        ...
        {For k := t To 2*t Do
          ...
          End;}
        if (l <= r) Then Begin
        sum(a[i, j], a[i-l, j-1], d);
        vych(d, a[i-r - 1, j-1], a[i, j]);
        End;
        sum(a[i, j], a[i-1, j], d);
        swap(a[i, j], d);
      End;
  End;
  For j := 1 To n Div t Do
    For i := n downto 1 Do
      Begin
        vych(a[i, j], a[i-1, j], d);
        swap(a[i, j], d);
      End;
End;
procedure find_c;
