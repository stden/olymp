...
var s,port,idd,psd,ip:string;
    i,j,x,kolp:integer;
    p,from,too,id,ps:array[0..50000]of string;
    bol:boolean;
label 1;
...
begin
  ...
  for i:=1 to length(s) do begin
    if s[i] in ['0'..'9'] then begin
      ...
      if (s[i]='|')and(kol=4) then mask:=c else begin
        s1:=s1+strto2(c);
        c:=0;
        inc(kol);
      end;
  ...
  if length(s1)<>32 then result:=false;
  if length(s2)<>32 then result:=false;
...
begin
  result:=false;
  for i:=1 to length(id) do if not(id[i] in['0'..'9']) then exit;
  for i:=1 to length(idd) do if i<>length(id)+1 then if not(idd[i] in['0'..'9']) then exit;
