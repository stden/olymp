...
begin
 ...
 while curtime<0 do
  ...
  cc:=curtime;
    sres:='['+sres+':';
  sres:=sres+chr(ord('0')+cc div 600)+chr(ord('0')+(cc div 60)mod 10)+':';

  cc:=cc mod 60;
  sres:=sres+chr(ord('0')+cc div 10)+chr(ord('0')+cc mod 10);

  sres:=sres+':'+s[20]+s[21]+' '+tz+']';
  s:=sres;
  end;
 if not ok1 then
 while curtime>=1440 do
  begin
 ok1:=true;
   curtime:=curtime - 1440;
  cc:=curtime;
