...
type integer=longint;
     tarr=array[0..60]of integer;
var n,t,i,j,k:integer;
    b,c:array[0..500] of array of tarr;
...
begin
     ...
     for i:=1 to n do begin
         for j:=i to n do begin
             b[i,j-i]:=zero;
             for k:=t-1 to min(2*t-1,j) do if j-k-i+1>=0 then
                 b[i,j-i]:=plus(b[i,j-i],b[i-1,j-k-i+1]);
         end;
     end;
     c[0,0]:=one;
     for i:=1 to n do begin
         for j:=i to n do begin
             c[i,j-i]:=zero;
             for k:=t to min(2*t,j) do if j-k-i+1>=0 then
                 c[i,j-i]:=plus(c[i,j-i],c[i-1,j-k-i+1]);
         end;
     end;
     a[1]:=one;
     for i:=2 to n do begin
         ...
         if i=100 then
            a[i]:=zero;
         for j:=1 to i-1 do a[i]:=plus(a[i],mnog(c[j,i-j],a[j]));
     end;
     ans:=zero;
     for i:=1 to n do ans:=plus(ans,mnog(b[i,n-i],a[i]));
     if ans[0]=0 then writeln(0) else begin
        for i:=ans[0] downto 1 do begin
            if i<>ans[0] then begin
               if ans[i]<1000 then write(0);
