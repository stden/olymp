...
void pushFlow(int a, int b) {
	...
	w[a] -= to; w[b] += to;
	f[a][b] += to; f[b][a] -= to;
	if(w[b]==to && to>0 && b!=s && b!=t)q.insert(mp(h[b],b));
}
void init() {
	q.clear();
	h = pos = VI(n, 0);
	w = vector<int64> (n, 0);
	h[s] = n; w[s] = (int64)n*INF;
	forn(i,n)
		forn(j,n)f[i][j]=0;
...
void up(int v) {
	while (pos[v] < n) {
		if(c[v][pos[v]]-f[v][pos[v]]>0)pushFlow(v, pos[v]);
		if (w[v] == 0) return ;
		++pos[v];
	}
	pos[v] = 0;
	h[v]=3*n;
	forn(i,n)
	  if(c[v][i]-f[v][i]>0)h[v]=min(h[v],h[i]+1);
	up(v);
	//q.insert(mp(h[v],v));
}
void flow() {
 	...
 	forn(i,kol){
		...
		forn(j,n)
			if(now[j]==-1){
				now[j]=best[j] ^ 1;
				add(now);
				if((int)u.size()>kol-i)u.erase(--u.end());
				now[j]=best[j];
				}
	       	}
       return 0;
