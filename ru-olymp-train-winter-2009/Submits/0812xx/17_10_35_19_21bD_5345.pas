...
Var x,t1,sec,j,t,hh,mm,ss,h1,m1,s1,i,l,speed:Integer;
    ...
    tecRead,tecWrite,SecToPay,TotSec,TotRead,TotWrite:Integer;
...
Begin
  ...
  while not EoF do begin
    ...
    if (s[3]<>'-') or (s[6]<>'-')or(s[24]<>'-') then continue;
    ...
    if (hh>23)or(mm>59)or(ss>59) then begin {Sdohni;}continue;end;
    ts:='';
    ...
    if (pos('Writes',ts))<>0 then begin
      ...
      for i:=t to t1 do x:=x*10+CharToDigit(ts[i]);
		...
		if(ts='Standard Modem closed.') then begin
			if speed<>-1 then begin totRead:=totRead+tecRead;
			totWrite:=totWrite+tecwrite;
			WriteLn(tecRead,'/',tecWrite);
			speed:=-1;continue;end;
		end;
  end;
  WriteLn('Total seconds to pay = ',SecToPay,', total seconds = ',TotSec,'; total bytes ',TotRead,'/',TotWrite);
End.