...
begin
     ...
     for i:=1 to n-1 do begin
         ...
         for j:=1 to n do if b[j]=0 then
             for k:=1 to n do if (abs(a[j,k])<mx)and(abs(a[j,k])>eps) then begin
                 mx:=abs(a[j,k]);
                 mi[i]:=j;
                 mj[i]:=k;
             end;
         b[mi[i]]:=1;
         for j:=1 to n+1 do a[mi[i],j]:=a[mi[i],j]/a[mi[i],mj[i]];
         a[mi[i],mj[i]]:=1;
     ...
     for i:=1 to n do begin
         bol:=true;
         for j:=1 to n do if abs(a[i,j])>eps then bol:=false;
         if (bol)and(abs(a[i,n+1])<eps) then begin
            writeln('infinity');
            halt;
         end;
         if (bol)and(abs(a[i,n+1])>=eps) then begin
