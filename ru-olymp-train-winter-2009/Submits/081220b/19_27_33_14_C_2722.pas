...
var
  a,b:Tlong;
  l,r,ans:Tlong;
  med,one,help,ghelp:Tlong;
  i,j,kol:longint;
  start,rk,ostart,prk:longint;
...
begin
  ...
  for i:=ans[0] downto 1 do if ans[i]>0 then begin
    ...
    if rk<>-1 then begin
      ...
      for j:=ghelp[0] downto 1 do
        if ghelp[j]>med[med[0]-(ghelp[0]-j)] then begin
          inc(rk);
          break;
         end else
        if ghelp[j]<med[med[0]-(ghelp[0]-j)] then break;
      start:=a[0]-med[0];
    end;
