...
char ct[10];
char s[400];
int len, pay, time, bin, bout, o, speed, in1, out1;
map <string, int> m;
string mn[12];
...
int check2 (string s)
{
  if (s.length() < 6)
    return -1;
  if (s.substr (s.length()-6, 6) != " bytes")
    return -1;
  ...
  if (st == "Hanging up the modem." && o == 1)
    {
      o = 2;
      st = s;
      int tm1 = count1 (st, st1);
      cerr << tm1 << endl;
      printf ("%d %d ", tm1, speed);
      time += tm1;
      if (tm1 >= 60)
        pay += tm1;
    }
