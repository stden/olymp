...
int main()
{
	...
	while(!fin.eof())
	{
		...
		if(r)
		{
			...
			if(s.find("Connection established at") != -1)
			{
				...
				v.push_back(con());
				n++;
				v[n-1].start = ms + s*100 + m*100*60+h*100*60*60;
			...
			if(s.find("Hanging up the modem.") != -1 && connected)
			{
				...
				int mm,d,y,h,m,s,ms;
				sscanf(str, "%d-%d-%d %d:%d:%d.%d",&mm,&d,&y,&h,&m,&s,&ms);
				v[n-1].end = ms + s*100 + m*100*60+h*100*60*60;
			}
	...
	for(int i = 0; i < v.size(); i++)
	{
		...
		if(v[i].start < v[i].end)
			...
			tm = v[i].end +24*60*60*100 -v[i].start;
		tm= round(double(tm) / 100);
	...
	cout << "Total seconds to pay = " << tstp << ", total seconds = " << ts << "; total bytes " << r <<"/" << w;
	return 0;
}