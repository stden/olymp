...
/*  while (gets (ss)) {    
  ...
  while (cin >> s) {  
    ...
    for (int i = 0; i < tmp2.size (); i++) {
      if (tmp2[i] >= 'A' && tmp2[i] <= 'Z') tmp2[i] = tmp2[i] - 'A' + 'a';
    }
    tmp += tmp2;
    cur += tmp2.size ();
  }
  cur = tmp.size ();
  if (cur > 20000) { out ("NO"); return 0; }
    if (tmp.find (".in'") < tmp.size ()) { out ("NO"); return 0;}
    if (tmp.find (".out'") < tmp.size ()) { out ("NO"); return 0;}
    if (tmp.find ("object") < tmp.size ()) { out ("YES"); return 0;}        
    if (tmp.find ("unit") < tmp.size ()) { out ("YES"); return 0;}        
    if (tmp.find ("run") < tmp.size ()) { out ("YES"); return 0;}        
//     
//    if (tmp.find ("}") < tmp.size ()) { out ("NO"); return 0;}        
//    if (tmp.find ("//") < tmp.size ()) { out ("NO"); return 0;}            
    if (tmp.find ("решен") < tmp.size ()) { out ("YES"); return 0;}
    if (tmp.find ("автор") < tmp.size ()) { out ("YES"); return 0;}
    if (tmp.find ("задач") < tmp.size ()) { out ("YES"); return 0;}
    if (tmp.find ("Решен") < tmp.size ()) { out ("YES"); return 0;}
    if (tmp.find ("Автор") < tmp.size ()) { out ("YES"); return 0;}
    if (tmp.find ("Задач") < tmp.size ()) { out ("YES"); return 0;}
    if (tmp.find ("jury") < tmp.size ()) { out ("YES"); return 0;}
