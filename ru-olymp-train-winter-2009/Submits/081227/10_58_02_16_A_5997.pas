...
begin
   ...
   begin
     ...
     if containing(S,'.in') then
       Verdict:='NO!';
     if containing(S,'.out') then
       Verdict:='NO!';

     if containing(S,'assign') then
       NAss:=NAss+1;
     if containing(S,'reset') then
       NR:=NR+1;
     if containing(S,'rewrite') then
       NR:=NR+1;
     if containing(S,'close') then
       NC:=NC+1;
     ...
     if (True{Verdict='YES'})and(abs(M-N)mod 4<>0)
     then
       Verdict:='NO';
