 ...
 */
public class vampire_ek {
	public static void main(String[] args) {
		InputStream inputStream;
		try {
			inputStream = new FileInputStream("vampire.in");
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
		OutputStream outputStream;
		try {
			outputStream = new FileOutputStream("vampire.out");
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
		InputReader in = new InputReader(inputStream);
		OutputWriter out = new OutputWriter(outputStream);
		Vampire solver = new Vampire();
...
class Vampire {
	public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		Map<Integer, Pair<Integer, Integer>> answer = new EHashMap<Integer, Pair<Integer, Integer>>();
		if (digits >= 6) {
			for (int i = 100; i < 999; i++) {
				for (int j = i; j < 999; j++) {
					int total = i * j;
					if (total < 100000 || answer.containsKey(total))
						continue;
					int[] perDigit = new int[10];
					int iCopy = i;
					while (iCopy != 0) {
						perDigit[iCopy % 10]++;
						iCopy /= 10;
					...
					if (ok) {
						answer.put(total, Pair.makePair(i, j));
						if (answer.size() == count) {
							for (int k : answer.keySet()) {
								out.print(k);
								for (int l = 6; l < digits; l++)
									out.print(0);
								out.print('=');
								out.print(answer.get(k).first);
								for (int l = 3; l * 2 < digits; l++)
									out.print(0);
								out.print('x');
								out.print(answer.get(k).second);
								for (int l = 3; l * 2 < digits; l++)
									out.print(0);
								out.printLine();
							}
			...
			for (int i = 10; i < 99; i++) {
				for (int j = i; j < 99; j++) {
					int total = i * j;
					if (total < 1000 || answer.containsKey(total))
						continue;
					int[] perDigit = new int[10];
					int iCopy = i;
					while (iCopy != 0) {
					...
					if (ok) {
						...
						if (answer.size() == count) {
							for (int k : answer.keySet()) {
								out.print(k);
								for (int l = 4; l < digits; l++)
									out.print(0);
								out.print('=');
								out.print(answer.get(k).first);
								for (int l = 2; l * 2 < digits; l++)
									out.print(0);
								out.print('x');
								out.print(answer.get(k).second);
								for (int l = 2; l * 2 < digits; l++)
