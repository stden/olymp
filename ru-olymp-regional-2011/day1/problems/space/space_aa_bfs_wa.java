...
public class space_aa_bfs_wa {
	public static void main(String[] args) {
		new space_aa_bfs_wa().run();
	}
	...
	private void solve() throws IOException {
		...
		a[k][1][start] = 0;
		Queue<State> queue = new ArrayDeque<space_aa_bfs_wa.State>();
		...
		for (int i = 0; i < x.length; i++) {
			for (int j = -k + 1; j <= k - 1; j++) {
				int xx = x[i] + j;
				int yy = y[i];
				if (0 <= xx && xx < a[yy].length && yy == maxy) {
					ans = Math.min(ans, a[yy][xx][prof]);
				}
			}
		}
		out.println(ans);
