...
public class house_aa_nlogn_wa {
	public static void main(String[] args) {
		new house_aa_nlogn_wa().run();
	}
	...
	private void solve() throws IOException {
		...
		Arrays.sort(a);
		{
			ArrayList<SemiPlane> tmp = new ArrayList<house_aa_nlogn_wa.SemiPlane>();
		...
		for (int step = 0; step < 50; step++) {
			double d = (l + r) / 2;
			for (int i = 0; i < a.length; i++) {
				al[buff[2 * i].number] = buff[2 * i];
				al[buff[2 * i + 1].number] = buff[2 * i + 1];
