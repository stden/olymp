...
set exec=run  -m 512M %problem%_%author%.exe
)
...
set exec=run -m 512M  java -ea -Xss64M -Xmx512M %problem%_%author%
)
if EXIST %problem%_%author%.py (
set exec=run -m 512M python %problem%_%author%.py
)
...
for %%i in (tests\??) do (
	...
	if exist Check.class java -Xmx512M -Xss64M -cp ../../lib/testlib4j.jar;. ru.ifmo.testlib.CheckerFramework Check %%i %problem%.out %%i.a
    if errorlevel 1 (
